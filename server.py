...
def sent_analyzer():
    ...
    return f"For the given statement, the system response is {keys[0]}: {response[keys[0]]}, {keys[1]}: {response[keys[1]]}, {keys[2]}: {response[keys[2]]}, {keys[3]}: {response[keys[3]]} and {keys[4]}: {response[keys[4]]}. The dominant emotion is {response['dominant_emotion']}"
